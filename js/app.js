const App = (() => {
  ...
  function init(){
    ...
    $('#loginSubmit').addEventListener('click', async () => {
      ...
      if(v.online && v.match.id){
        try {
          ...
          if(encryptedBank && encryptedBank.bankId){
            ...
            if(cloudBank && cloudBank.encrypted_data){
              try {
                const decrypted = await Crypto.decrypt(cloudBank.encrypted_data, bankPwd);
                bank = decrypted;
                showToast('✅ 题库已解锁', 'ok');
              } catch(err){
      ...
      if(!bank){
        bank = v.match.bankSnapshot;
        match = v.match;
      }
      if(!bank){
        status.textContent = '❌ 找不到题库数据';
        status.style.color = 'var(--danger)';
        return;
      }
      ...
      if(window.Realtime) window.Realtime.setMatch(match.id || state.playerKey);

      let progress = Storage.getProgress(state.playerKey);
      if(!progress){
        progress = TriggerEngine.initProgress(bank, ['现勘']);
      }
      ...
      if(isOnline() && match.id){
        ...
        } catch(err){ console.warn('云端进度上传失败:', err); }
      }
